...
export function seedDemoData(syncGraph = false): Promise<{ status: string; counts: Record<string, number>; graph?: { nodes: number; relationships: number; cases: number } | null; graph_warning?: string | null }> { return request<{ status: string; counts: Record<string, number>; graph?: { nodes: number; relationships: number; cases: number } | null; graph_warning?: string | null }>(`/api/workspace/demo/seed?sync_graph=${syncGraph ? "true" : "false"}`, { method: "POST" }); }
export function syncDemoGraph(reset = false): Promise<{ status: string; nodes: number; relationships: number; cases: number }> { return request<{ status: string; nodes: number; relationships: number; cases: number }>(`/api/workspace/demo/sync-graph?reset=${reset ? "true" : "false"}`, { method: "POST" }); }
